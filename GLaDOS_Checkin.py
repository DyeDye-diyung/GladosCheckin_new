...
if __name__ == '__main__':
    ...
    # 5. PushPlus 统一推送
    push_title = f"GLaDOS 自动签到报告: 成功{success_count}, 失败{fail_count}"
    logging.info("发送推送通知...")
    try:
        push_url = f"http://www.pushplus.plus/send?token={config.pushplus_token}&title={push_title}&content={summary_content}"
        session.get(push_url, timeout=30)
        print(f"任务完成，已推送报告。\n{summary_content}")
    except Exception as e:
